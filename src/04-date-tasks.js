...
function isLeapYear(date) {
  const februaryDays = new Date(date.getFullYear(), 2, 0);
  return februaryDays.getDate() === 29;
}
...
function timeSpanToString(startDate, endDate) {
  ...
  const minutes = (allMinutes % 60).toString().padStart(2, '0');
...
function angleBetweenClockHands(date) {
  const hoursUTC = date.getUTCHours() % 12;
  const minutesUTC = date.getUTCMinutes();
  const hoursAngle = (hoursUTC * 30 + minutesUTC * 0.5) % 360;
  const minutesAngle = (minutesUTC * 6) % 360;
